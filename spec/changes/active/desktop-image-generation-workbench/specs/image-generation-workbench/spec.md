...
生图工作台 MUST 采用桌面应用信息架构：批次队列作为常驻左栏（含新建批次入口与每批进度），生成配置面板可折叠且在建批次流程外不占据画廊空间，画廊头部 MUST 显示当前批次身份并提供直接切换，图片单击 MUST 打开大图查看器（键盘左右切换、收藏/下载/复制提示词/加入 Prompt 操作），进行中的批次 MUST 在画廊内提供不依赖批次抽屉的进度可见性。禁止保留无内容的"Advanced"开关等误导性控件；多选 MUST 遵循桌面语义（单击主选、Shift/Ctrl 加选），选择标记仅在悬停或已选时显示。

#### Scenario: 切换批次

Given 画廊正在显示批次 A
When 用户在左栏选择批次 B
Then 画廊与头部立即切换到批次 B，无遮罩层遮挡配置面板
