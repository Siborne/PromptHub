...
生图工作台 MUST 采用画廊优先的两栏桌面信息架构：结果画廊占据主要空间，右侧固定检查器在“生成设置”和“历史作品”之间切换；历史不得作为常驻第三栏挤压画廊。进入工作台后，Prompts 全局模块入口保留，但文件夹、标签等二级侧栏 MUST 自动收起，为结果审阅释放横向空间；退出工作台后恢复既有侧栏行为。头部 MUST 把工作台身份、批次切换、新建批次和历史入口保持在一行，把作品筛选与画廊选项放在独立工具行，避免标题、筛选和操作互相覆盖。
