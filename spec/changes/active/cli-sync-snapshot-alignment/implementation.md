...
The standalone CLI still lacks first-class remote sync commands such as `prompthub sync status/push/pull` for self-hosted or cloud targets. This change aligns the snapshot contract and Cloudflare preservation layer; it does not complete the remote sync command UX.
