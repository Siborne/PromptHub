...
Provider & Model 页保留左右分栏(左:供应商 Profile 列表;右:具体配置),但配置体验对齐
...
- 左侧:Profile 列表(名称 + 主模型 + current 文字标记),顶部 `[+ 新增]` 与 `[导入当前配置]`。
  列表行不承载彩色徽章和卡片阴影,保持与设计 token 一致的轻量视觉。
- 右侧:选中 Profile 即进入配置表单(新增时先出现预设选择,选定后进入同一表单)。
...
5. 保存成功后 Profile 进入列表,可立即执行测试/激活。

### Flow B:导入当前配置

1. 左侧 `[导入当前配置]` → main 进程 adapter 解析原生配置 → 脱敏预览。
2. 确认后创建 Profile,右侧直接进入表单编辑。

### Flow C:编辑

- 选中 Profile → 右侧表单直接可编辑;取消丢弃,保存走 `expectedUpdatedAt` 乐观并发。
