...
- Canonical Plugin target projections are device-local metadata scoped to the active PromptHub storage root. Their identity is derived from the stable storage-root identity, not from the optional self-hosted sync identity, so Plugin reads and migration work before sync setup and remain stable across synchronization configuration changes.
