...
| #176  | open         | accepted        |                |                                              | The reported markerless `~/.claude/get-shit-done` bundle is still skipped because manual Claude package scanning requires `package.json`; add an exact regression before broadening detection safely. |
| #175  | open         | released        | 0.5.9-beta.2   | `mcp-management`                             | The requested AGHub-style My/Agent/Store Skill and MCP workbenches are present in the published beta; the GitHub issue is eligible for closure.                                                       |
| #167  | open         | accepted        |                |                                              | Store search is currently exposed only for skills.sh and ClawHub; custom Skill Store sources still have no search control despite shared filter support.                                              |
